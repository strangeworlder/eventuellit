import { j as h } from "./jsx-runtime-u17CrQMm.js";
import { t as v, c as y } from "./bundle-mjs-Ce1ZTWB2.js";
import { R as s } from "./iframe-RLsCwdXb.js";
function f(...e) {
  return v(y(e));
}
const b = ({ text: e }) => {
    const [n, d] = s.useState(null),
      [m, u] = s.useState({ center: 0, toggle: !1 });
    return (
      s.useEffect(() => {
        if (!e) return;
        let a,
      ...
      s.useEffect(() => {
        ...
        u({ center: a[Math.floor(Math.random() * a.length)] ?? 0, toggle: !1 });
        const t = (e.length * 0.15 + 4) * 1e3,
          i = Math.max(1e4, t + 500),
          l = setInterval(() => {
            u((c) => ({ center: a[Math.floor(Math.random() * a.length)] ?? 0, toggle: !c.toggle }));
          }, i);
        return () => clearInterval(l);
      }, [e]),
      h.jsx(h.Fragment, {
        children: Array.from(e).map((a, r) => {
          const o = Math.abs(r - m.center),
            t = n === r;
          return h.jsx(
            "span",
            {
              className: f(
                t ? "flicker-char" : "",
                m.toggle ? "breath-char-1" : "breath-char-2",
                "inline-block transition-colors duration-300",
              ),
              style: { animationDelay: `${o * 0.15}s` },
              children: a === " " ? " " : a,
            },
            r,
          );
  ...
  w = (e) => s.Children.map(e, (n) => (typeof n == "string" ? h.jsx(b, { text: n }) : n)),
  p = s.createContext(2),
  k = ({ children: e }) => {
    const n = s.useContext(p),
      d = Math.min(n + 1, 6);
    return h.jsx(p.Provider, { value: d, children: e });
  },
  g = s.forwardRef(({ className: e, as: n, variant: d, children: m, ...u }, a) => {
    const r = s.useContext(p),
      o = n || `h${Math.min(Math.max(1, r), 6)}`,
      t = d || o;
    let i = m;
    return (
      t === "h1" && (i = w(m)),
      h.jsxs(o, {
        ...
        className: f(
          ...
          {
            "font-black tracking-normal": ["h1", "h2", "h4", "h5"].includes(t),
            "font-heading text-4xl tablet:text-5xl uppercase text-[var(--theme-primary)]":
              t === "h1",
            "font-heading text-3xl tablet:text-4xl uppercase text-[var(--theme-text)]": t === "h2",
            "font-heading text-2xl tablet:text-3xl uppercase text-[var(--theme-primary)]":
              t === "h3",
            "font-sans text-xl tablet:text-2xl border-l-4 pl-4 border-[var(--theme-primary)] uppercase text-[var(--theme-text)]":
              t === "h4",
            "font-sans text-lg tablet:text-xl uppercase text-[var(--theme-secondary)]": t === "h5",
...
k.__docgenInfo = {
  description: "",
  methods: [],
  displayName: "HeadingLevelProvider",
  props: {
    children: {
      required: !0,
...
export { k as H, g as a, p as b, f as c };
