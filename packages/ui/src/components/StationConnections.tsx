...
export const StationConnections = React.forwardRef<HTMLDivElement, StationConnectionsProps>(
  function StationConnections(
    ...
    const [displayStationOrder, setDisplayStationOrder] = useState(currentStationOrder);
    ...
    useEffect(() => {
      ...
      const ro = new ResizeObserver(() => setContainerPx(el.offsetWidth || SIZE));
      ro.observe(el);
    ...
    const displayNodes = displayConnections.map(({ title, direction, type, shape }) => {
      ...
      return {
        ...
        station: isMarker
          ? null
          : (stations.find((s) => s.title.toLowerCase() === title.toLowerCase()) ?? null),
      };
    });
    ...
    const heroNode =
      phase === "exit" && heroDir
        ? (displayNodes.find((n) => n.direction === heroDir && !n.isMarker) ?? null)
        : null;

    // Pan offset (CSS pixels): scene moves in opposite direction to exit dir
    const exitAngle = heroDir ? COMPASS_ANGLES[heroDir] : 0;
    const panX = phase === "exit" && heroDir ? -Math.cos(exitAngle) * PAN_DISTANCE * scale : 0;
    const panY = phase === "exit" && heroDir ? -Math.sin(exitAngle) * PAN_DISTANCE * scale : 0;

    // ── Event handlers ────────────────────────────────────────────────────────

    const handleStationClick = (station: ConnectedStation, direction: CompassDir) => {
      if (phase !== "idle") return;
      exitDirRef.current = direction;
      if (onNavigate) {
        onNavigate(station.id);
      } else {
        const href = basePath === "/" ? `/${station.id}` : `${basePath}/${station.id}`;
        navigate(href);
      }
    };
    ...
    const gradientDefs = (nodes: typeof displayNodes, centerColor: string) => (
      <defs>
     ...
     */
    const staticLines = (nodes: typeof displayNodes, excludeTitle?: string) =>
      nodes
    ...
    const nodeContent = (node: (typeof displayNodes)[0], interactive: boolean) => {
      const color = tensionColor(node.station?.tension);
      const placement = tooltipPlacement(node.direction);

      if (node.isMarker) {
        const isD12 = node.shape !== "swirl";
        const markerColor = isD12 ? "var(--color-royal-purple-500)" : "var(--theme-text)";
        const dice = (
          <DiceIcon
      ...
      return (
        <div className="group flex flex-col items-center gap-1 relative">
          ...
          <Text variant="station-node" style={{ color }}>
            {node.title}
          </Text>
        </div>
      );
            ...
            <>
              ...
              >
                ...
                {displayNodes
                  ...
                  .map((node) => (
                    <div key={node.title} style={{ ...posStyle(node.x, node.y), zIndex: 10 }}>
                ...
                <div style={{ ...posStyle(CX, CY), zIndex: 20 }}>
                  ...
                  >
                    <DiceIcon
                      ...
                      style={
                        {
                          "--theme-primary": tensionColor(heroNode.station?.tension),
                          "--theme-secondary": tensionColor(heroNode.station?.tension),
                        } as React.CSSProperties
                      }
                    />
                  </motion.div>
          ...
          {phase === "enter" && (
            <>
              ...
              {displayNodes.map((node, i) => {
                const entryX = Math.cos(node.angle) * RADIUS * 0.75 * scale;
                const entryY = Math.sin(node.angle) * RADIUS * 0.75 * scale;
                return (
                  <div key={node.title} style={{ ...posStyle(node.x, node.y), zIndex: 10 }}>
                    <motion.div
                      ...
                      onAnimationComplete={i === lastNodeIndex ? handleAllEnterComplete : undefined}
                    >
                      {nodeContent(node, false)}
                    </motion.div>
                  </div>
