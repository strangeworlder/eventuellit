...
export function ImageField({
  ...
  const [activeTab, setActiveTab] = useState<Tab>(mediaItems.length > 0 ? "library" : "upload");
    ...
    <div className={cn("flex flex-col gap-2 mt-2 w-full", className)}>
      ...
      {value && (
        <div className="relative overflow-hidden rounded-sm border-2 border-[var(--theme-border-medium)] bg-[var(--theme-bg)]">
          <img src={value} alt="" className="w-full h-40 object-cover" loading="lazy" />
        </div>
      )}
            ...
            <div className="grid grid-cols-3 tablet:grid-cols-4 gap-2 max-h-[280px] overflow-y-auto p-1">
              {mediaItems.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => onChange({ url: item.publicUrl, mediaId: item.id })}
                  className={cn(
      ...
      {activeTab === "upload" && (
        ...
        >
          {uploading ? (
            <>
              <Icon name="loader2" size={24} className="text-[var(--theme-primary)] animate-spin" />
              <span className="text-sm text-text-muted font-bold">Optimoidaan ja ladataan...</span>
            </>
          ) : (
            <>
              ...
              <span className="text-xs text-text-muted">PNG, JPG tai WebP — enintään 15 Mt</span>
            </>
          )}
          <input
            ref={fileInputRef}
