...
export const CardHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { theme?: Theme }>(
  ({ className, theme, ...props }, ref) => (
    <div
      ref={ref}
      data-theme={theme}
      className={cn("flex flex-col space-y-1.5 p-6 border-b-2 border-[var(--theme-primary)]/20", className)}
      {...props}
    />
  ),
);
...
>(({ className, theme, ...props }, ref) => (
  <p ref={ref} data-theme={theme} className={cn("text-lg font-bold uppercase tracking-wider text-[var(--theme-text)]", className)} {...props} />
));
...
>(
  ({ className, variant = "default", theme, ...props }, ref) => (
    <div ref={ref} data-theme={theme} className={cn("flex flex-col",
      {
