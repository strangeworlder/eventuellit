...
export interface TextProps extends React.HTMLAttributes<HTMLParagraphElement> {
   ...
   * - `label`: Section labels & category headers (heading font, uppercase, tracked).
   * - `caption`: Tiny metadata & footnotes.
   * - `overline`: Uppercase tracked label in primary accent (heading font, section kicker).
   * - `kicker`: Tiny uppercase section labels (heading font, placeholder contrast).
   * - `timestamp`: Micro-sized tabular-nums metadata (relative times, dates).
 ...
 *
 * @summary body/UI text with variant scale: body, lead, muted, small, bold, label, caption, timestamp, node
 */
      ...
      <p
        ...
        className={cn(
          "font-sans text-[var(--theme-text)] tracking-[0.015em]",
          {
            ...
            "font-heading text-[length:var(--font-size-xs)] font-black uppercase tracking-widest":
              variant === "label",
            "text-[length:var(--font-size-xs)] leading-snug text-[var(--theme-text-subtle)]":
              variant === "caption",
            "font-heading text-[length:var(--font-size-xs)] font-black uppercase tracking-widest text-[var(--theme-primary)]":
              variant === "overline",
            "font-heading text-[length:var(--font-size-3xs)] font-black uppercase tracking-widest text-text-placeholder":
              variant === "kicker",
            "text-[length:var(--font-size-2xs)] tabular-nums text-[var(--theme-text-subtle)]":
              variant === "timestamp",
