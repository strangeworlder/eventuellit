...
export const DicePoolAllocator = React.forwardRef<HTMLDivElement, DicePoolAllocatorProps>(
    ({ className, maxDice, axes, attributeDie, onAllocationChange, availableDiceLabel = "Käytettävissä olevat nopat", theme, ...props }, ref) => {
        ...
        return (
            <div
                ref={ref}
                data-theme={theme}
                className={cn("p-8 rounded-sm border-4 border-[var(--theme-primary)]/40 bg-[var(--theme-bg)] space-y-8 shadow-lg relative overflow-hidden", className)}
                {...props}
            >
                <HeadingLevelProvider>
                    ...
                    <div className="flex flex-nowrap justify-between items-end border-b-2 border-[var(--theme-primary)]/30 pb-4 relative z-10 gap-4">
                        <div className="w-full sm:w-auto">
                            ...
                            <p className="text-[var(--theme-text)]/70 text-base font-bold mt-1">Jaa nopat vaadittujen ominaisuuksien kesken.</p>
                        </div>
                        <div className="text-left sm:text-right w-full sm:w-auto">
                            <span className="block text-5xl font-heading font-black text-[var(--theme-text)]">{diceRemaining} <span className="text-2xl text-[var(--theme-text)]/40">/ {maxDice}</span></span>
                            <span className="text-sm text-[var(--theme-primary)] font-bold uppercase tracking-widest mt-1 block">{availableDiceLabel}</span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <HeadingLevelProvider>
                            {axes.map(axis => (
                                <div key={axis} className="flex flex-wrap items-center justify-between p-4 rounded-sm bg-[var(--theme-secondary)]/5 border-2 border-[var(--theme-secondary)]/20 relative z-10 gap-4">
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 w-full lg:w-auto flex-1 min-w-[200px]">
                                        ...
                                        <div className="flex flex-wrap gap-2 items-center flex-1 min-h-[40px]">
                                            {Array.from({ length: allocation[axis] || 0 }).map((_, i) => (
                                                <DiceIcon key={i} faces={20} size="md" active={true} className="-rotate-3 hover:rotate-0" />
                                            ))}
                                            {(allocation[axis] || 0) === 0 && <span className="text-sm text-[var(--theme-text)]/30 italic font-bold whitespace-nowrap">-- Ei noppia --</span>}
                                        </div>
                                    </div>
