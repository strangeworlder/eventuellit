...
## Hahmopaja (Character Generator) Status

Currently disabled in production. To re-enable:
- Search for `"Hahmopaja temporarily hidden"` and `"temporarily disabled"` comments in `apps/host/src/App.tsx` and `apps/host/src/components/LandingPage.tsx`.
- Uncomment/restore relevant code.
- Restore `Dice5` to the lucide-react import in `App.tsx`.
- Revert landing page grid back to `desktop:grid-cols-3` with `max-w-6xl`.
